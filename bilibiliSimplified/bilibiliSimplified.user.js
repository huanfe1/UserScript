...
(function () {
    ...
    function setRecommend() {
        ...
        .switch_button{width:35px;height:20px;background:#9499A0;cursor:pointer;border-radius:10px;margin-left:15px;position:relative}
        .switch_button::before{content:"";position:absolute;top:2px;left:3px;border-radius:100%;width:16px;height:16px;background-color:#fff;transition:all .2s}
    ...
    function addRecommend() {
        ...
        let text = `
                <a href="${item.uri}" target="_blank">
                    ...
                    <div class="timeshow">${timeConversion(item.duration)}</div>
                </a>
                <div class="bilibili_recommend_text"><a href="${item.uri}">${item.title}</a></div>
                <div class="recommend_info">
                    <a href="https://space.bilibili.com/${item.owner.mid}" class="playinfo" target="_blank">
        ...
        hoverPreview(dom, 300, item.id);
    }

    function getRecommendData(num) {
        // 获取推荐API数据
        GM_xmlhttpRequest({
            method: "GET",
            ...
            onload: function (response) {
                recommendData.push.apply(recommendData, JSON.parse(response.responseText).data.item);
            },
            onerror: function () {
